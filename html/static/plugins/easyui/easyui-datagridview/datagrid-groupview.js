...
var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
	...
	renderGroup: function(target, groupIndex, group, frozen){
		...
		if ((frozen && (opts.rownumbers || opts.frozenColumns.length)) ||
			...
			if(opts.showexpander!=null){
				if($.isFunction(opts.showexpander)){
					needshow = opts.showexpander.call(target, group.value, group.rows);
				}else{
					needshow = opts.showexpander;
				}
			}
			...
			if(needshow)
				...
				table.push('<span class="datagrid-row-expander datagrid-row-collapse" style="display:none">&nbsp;</span>');
	...
	onBeforeRender: function(target, rows){
		...
		for(var i=0; i<rows.length; i++){
			...
			var row = rows[i];
			var gv = getGroupFieldValue(row, gfs);
			
			var group = getGroup(gv);
			if (!group){
				group = {
					value: gv,
					rows: [row]
...
function getGroupFieldValue(row, groupfields){
	if(groupfields.length==1)
		return row[groupfields[0]];
	else{
		var vs = [];
		$.each(groupfields, function(idx, v){
			vs.push(row[v]);
		});
